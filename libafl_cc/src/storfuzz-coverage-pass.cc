...
bool StorFuzzCoverage::runOnModule(Module &M) {
  ...
  auto THRESHOLD = 0;
  if(auto env_str = getenv("MAX_STORES_PER_BB")) {
          ...
          if ((storeInst = dyn_cast<StoreInst>(&instr))) {
            ...
            if (!(dyn_cast<AllocaInst>(storeLocation))) {
              ...
              if ((storedType =
                ...
                if (isa<LoadInst, VAArgInst>(actual_valueDefInstruction) && !getenv("STORFUZZ_INSTRUMENT_MEM2MEM_COPY")){
                  continue;
                ...
                if(!instrument_this_time) {
                  ...
                  if ((isa<PHINode>(storeLocation))) {
                    ...
                    PHINode *CurLocPhi = IRB.CreatePHI(Int32Ty,
                  ...
                  if(REDUCTION_WIDTH == 8) {
                    ...
                    ReducedValue = IRB.CreateXor(
                        IRB.CreateAnd(halfReduction, 0xF),
                    ...
                    auto temp = IRB.CreateLShr(IRB.CreateAnd(Lower16Bit, 0xFF00), 4);
